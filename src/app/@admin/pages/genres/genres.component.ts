...
export class GenresComponent implements OnInit {
  ...
  async takeAction($event) {
    ...
    switch (action) {
        ...
        case 'edit':
          ...
          break;
        case 'info':
          const result = await optionsWithDetails('Detalles', `${genre.name} (${genre.slug})`, 375,
  ...
  private async updateForm(html: string, genre: any) {
    const result = await formBasicDialog('Actualizar genero', html, 'name');
    console.log(result);
    this.updateGenre(genre.id, result);
  }

  private async blockForm(genre: any) {
    const result = await optionsWithDetails('¿Bloquear?', `Si bloqueas el item seleccionado, no se mostrara en la lista`,
      375, 'No', 'Si');
    if (result === false) {// Si resultado es falso, entonces queremos bloquear
